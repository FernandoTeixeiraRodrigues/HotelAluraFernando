...
public class ReservasView extends JFrame {
	...
	public static JComboBox<String> txtFormaPaga;
	int xMouse, yMouse;
	private JLabel labelExit;
	private JLabel lblValorSimbolo; 
	private JLabel labelAtras;

	private ReservasControles reservaControles;
	/**
	...
	public ReservasView() throws SQLException {
		...
		this.reservaControles = new ReservasControles();
		...
		txtFormaPaga = new JComboBox();
		txtFormaPaga.setBounds(68, 417, 289, 38);
		txtFormaPaga.setBackground(SystemColor.text);
		txtFormaPaga.setBorder(new LineBorder(new Color(255, 255, 255), 1, true));
		txtFormaPaga.setFont(new Font("Roboto", Font.PLAIN, 16));
		txtFormaPaga.setModel(new DefaultComboBoxModel(new String[] {"Cartão de Crédito", "Cartão de Débito", "Dinheiro"}));
		panel.add(txtFormaPaga);
	...
	private void guardaReserva() throws SQLException {
		
		if (txtDataE.getDate() !=null && txtDataS.getDate() !=null && !txtValor.equals("")
				&& !txtFormaPaga.getSelectedItem().toString().equals("") ) {
			
			LocalDate dataE = LocalDate.parse(((JTextField)txtDataE.getDateEditor().getUiComponent()).getText());
			LocalDate dataS = LocalDate.parse(((JTextField)txtDataS.getDateEditor().getUiComponent()).getText());
			Reservas reservas = new Reservas (  xMouse, dataE, dataS, txtValor.getText(), txtFormaPaga.getSelectedItem().toString());
			reservaControles.guardar(reservas);
	...
	public void calcularValor(JDateChooser dataE, JDateChooser dataS) {
		...
		if (dataE.getDate() != null  && dataS.getDate() != null) {
			if(dataE.getDate().after(dataS.getDate())) {
				...
				JOptionPane.showMessageDialog(null,"a data de entrada não pode ser posterior a data de saída",
						"Erro em datas", JOptionPane.ERROR_MESSAGE);
				return;
				}
